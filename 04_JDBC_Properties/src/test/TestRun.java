...
public class TestRun {
	...
	public static void main(String[] args) {
		...
		/*
		 * * Properties 특징
		 * - Map계열 컬렉션 (key + value 세트로 담는 특징)
		 * - key, value 모두 String(문자열)으로 담기
		 * - setProperty(String key, String value)
		 * - getProperty(String key) : String(value)
		 * 
		 * - 주로 외부 파일(.properties, /.xml)로 입출력할 때 사용 => 환경설정 파일 같은거.. 개발자가 아닌 사람도 읽을 수 있도록 하는 문서들
		 */
		...
		*/
		...
		System.out.println(prop.getProperty("passwosd")); // 존재하지 않는 키값 제시시 null 반환
